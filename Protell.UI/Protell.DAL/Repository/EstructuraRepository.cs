                    ...
                    if (result == null)
                    {
                        entity.CAT_ESTRUCTURA.AddObject(
                            new CAT_ESTRUCTURA()
                            {
                                ...
                                IdSistema = estructura.SISTEMA.IdSistema,
                                IsModified = true,
                    ...
                    (from o in entity.CAT_ESTRUCTURA
                     ...
                     select o).ToList().ForEach(p =>
                     {
                         Estructuras.Add(new Model.EstructuraModel()
                         {
                             ...
                             SISTEMA = new Model.SistemaModel()
                             {
                                 IdSistema = p.IdSistema,
                                 SistemaName = (p.CAT_SISTEMA != null)? p.CAT_SISTEMA.SistemaName : ""
                             }
                         });
                     });
                }
        ...
        public void UpdateEstructura(Model.EstructuraModel estructura)
        {
            using (var entity = new db_SeguimientoProtocolo_r2Entities())
            {
                ...
                if (result != null)
                {
                    ...
                    result.IdSistema = estructura.SISTEMA.IdSistema;
                    entity.SaveChanges();
                    _SyncRepository.UpdateSyn(entity);
                }
            }
            ...
            if (estructuras != null)
            {
                using (var entity = new db_SeguimientoProtocolo_r2Entities())
                {
                    foreach (Model.EstructuraModel item in estructuras)
                    {
                        ...
                        var modified = entity.CAT_ESTRUCTURA.First(p => p.IdEstructura == item.IdEstructura);
                ...
                using (var entity = new db_SeguimientoProtocolo_r2Entities())
                {
                    try
                    {
                        foreach (var item in listUnids)
                        {
                            ...
                            if (local.IdEstructura == item.IdTypeTable && local.LastModifiedDate <= item.LastModifiedDate)
                            {
